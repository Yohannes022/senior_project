...
import { StyleSheet, View, Text, ScrollView, TouchableOpacity, Image, Linking, Platform, Alert } from "react-native";
import { SafeAreaView } from "react-native-safe-area-context";
import { Stack, useRouter } from "expo-router";
import { ChevronLeft, ChevronRight, Mail, Globe, MessageSquare, Star, User, Shield, FileText, HelpCircle, Phone, MapPin, Clock } from "lucide-react-native";
...
export default function AboutScreen() {
  ...
  const openMap = () => {
    ...
    if (url) {
      ...
      openExternalLink(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(address)}`);
    }
  };
  ...
  const aboutItems = [
    {
      ...
      onPress: () => {
        ...
        const url = Platform.select({
          ...
          default: 'https://play.google.com/store/apps/details?id=com.shegertransit.app',
        });
        if (url) {
          Linking.openURL(url);
        }
  ...
  const appInfo = {
    ...
    deviceId: Platform.OS === 'android' ? Application.getAndroidId() : Application.getIosIdForVendorAsync?.() || 'N/A',
    os: Platform.OS,
    ...
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      <ScrollView style={styles.scrollView}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.appSlogan}>Your Reliable Urban Mobility Partner</Text>
        ...
        <View style={styles.section}>
          ...
          {contactItems.map((item) => (
            ...
            >
              ...
              {item.onPress && <ChevronRight size={20} color={Colors.textLight} />}
            </TouchableOpacity>
          ))}
        </View>
